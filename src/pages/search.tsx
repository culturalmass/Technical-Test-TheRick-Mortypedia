...
import { getChacterBySearch, getResponseByUrl } from "../actions/getCharacter";
import { DataCardProps, DataProps } from "./home";
...
const SearchPage = () => {
  //Set Use location to get the current path
  const location = useLocation();
  //Set the pagination on the Search page
  ...
  const handlePaginationForward = async () => {
    if (pagination.currentIndex >= pagination.size) {
      const newPage =
        data?.info?.next && (await getResponseByUrl(data?.info?.next));
      setData(newPage);
      setPagination({
        ...pagination,
  ...
  const handlePaginationBackward = async () => {
    if (pagination.currentIndex < pagination.size) {
      const newPage =
        data?.info?.prev && (await getResponseByUrl(data?.info?.prev));
      setData(newPage);
      setPagination({
        ...pagination,
        apiCallPage: pagination.apiCallPage - 1,
  ...
  //UseEffect to call the action getACharacterBySearch and set the data and the pagination in the state
  useEffect(() => {
    getChacterBySearch(location?.search?.split("?")[1]).then((data) => {
      setData(data);
  ...
  //Check is there was an error with the search request
  if (data?.error) {
    return (
      <div className="text-2xl font-kodeMono text-white text-muted-foreground m-12">
        {`No character found by ${location?.search?.split("?")[1]}!`}
      </div>
    );
  }
